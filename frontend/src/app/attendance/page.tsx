...
export default function AttendancePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const listData =
        (listRes?.data as Record<string, unknown>)?.attendance ??
        listRes?.data ??
        [];

      setRecords(Array.isArray(listData) ? listData : []);

      setStats((statsRes?.data ?? statsRes) as AttendanceStats);
    } catch (err) {
      setError(
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unable to save record"
      );
    }
  };
                ...
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {records.map((record, index) => (
                      <tr
                        key={record._id || `${record.date || "attendance"}-${index}`}
