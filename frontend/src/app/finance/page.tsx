  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setSummary((summaryRes?.data ?? summaryRes) as FinanceSummary);
        setMonthly(
          Array.isArray(monthlyRes?.data)
            ? (monthlyRes.data as MonthlyFinance[])
            : []
        );
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Unable to load finance data"
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            Review income, expenditures, and monthly trends at a glance.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <Card
              ...
              description="A simple financial overview layout"
            >
              {monthly.length === 0 ? (
                <EmptyState
                  title="No financial report available"
                  description="The finance module will populate once data is available."
                />
              ) : (
